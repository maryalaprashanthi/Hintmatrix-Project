...
const MockExamPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (examId) {
          const response = await MockExamService.getExamQuestions(examId);
  ...
  const runSubmit = async () => {
    ...
    try {
      ...
      console.groupCollapsed(
        `[mock exam submit] POST /api/mock-exams/${examId}/submit — ${payload.answers.length} question(s)`,
      );
      console.log("payload:", payload);
      console.log("payload (JSON):", JSON.stringify(payload, null, 2));
      console.groupEnd();

      const response = await MockExamService.submitExam(examId, payload);
  ...
  if (phase === "start") {
    return (
      <main className={`mockExamRoot ${styles.shell} ${styles.startShell}`} ref={shellRef}>
        <ExamStartScreen
          error={error}
          eyebrow={
            paper ? `${paper.courseName} · ${paper.chapterName}` : "Mock exam"
          }
          isLoading={loading}
          meta={[
  ...
  return (
    <main className={`mockExamRoot ${styles.shell}`} ref={shellRef}>
      <ExamTopBar
      ...
      {phase === "ended" &&
        (examId ? (
          <ExamResultDialog
            ...
            error={submitError}
            onExit={() => navigate("/mock-exams")}
            onRetry={runSubmit}
            onReview={
              result?.mockExamResultId
                ? () =>
                    navigate(`/mock-exams/${examId}/review/${result.mockExamResultId}`)
                : undefined
            }
            result={result}
            state={submitState}
